...
{
public:
  void onRosInitialized() override { initialized = true; }

  void onRosShutdown() override { initialized = false; }
...
TEST( RosLifeCycle, testLifeCycle )
{
  ...
  qml_ros_plugin::NodeHandle::Ptr qml_nh =
      std::make_shared<qml_ros_plugin::NodeHandle>( "test_ns" );
  ...
  EXPECT_EQ(
      tf_wrapper
          ...
          .toString(),
      QString( "Uninitialized" ) );
  auto publisher_before_init = dynamic_cast<qml_ros_plugin::Publisher *>(
      qml_nh->advertise( "geometry_msgs/PoseStamped", "/pose_stamped", 10 ) );
  ...
  auto publisher_after_init = dynamic_cast<qml_ros_plugin::Publisher *>(
      qml_nh->advertise( "geometry_msgs/PoseStamped", "/pose_stamped2", 10 ) );

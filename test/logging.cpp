...
TEST( Logging, log )
{
  ...
  EXPECT_EQ( static_cast<ros::console::Level>( ros_console_levels::Debug ),
             ros::console::levels::Debug );
  EXPECT_EQ( static_cast<ros::console::Level>( ros_console_levels::Info ),
             ros::console::levels::Info );
  EXPECT_EQ( static_cast<ros::console::Level>( ros_console_levels::Warn ),
             ros::console::levels::Warn );
  EXPECT_EQ( static_cast<ros::console::Level>( ros_console_levels::Error ),
             ros::console::levels::Error );
  EXPECT_EQ( static_cast<ros::console::Level>( ros_console_levels::Fatal ),
             ros::console::levels::Fatal );

  std::vector<std::pair<std::string, int>> log;
  ros::Subscriber rosout_sub = nh.subscribe<rosgraph_msgs::Log>(
      "/rosout", 0, [&log]( const rosgraph_msgs::LogConstPtr &msg ) {
        if ( msg->line != 0 )
          return;

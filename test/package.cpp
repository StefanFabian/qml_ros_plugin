...
::testing::AssertionResult listEquals( const ContainerA &container,
                                       const std::vector<KeyType> &values )
{
  std::map<KeyType, bool> found;
  for ( const auto &key : values ) { found.insert( { key, false } ); }
...
TEST( Package, package )
{
  ...
  EXPECT_TRUE( listEquals( package.getAll(), std::vector<QString>{ "poor_people", "rich_people",
                                                                   "rick_astley", "world" } ) );

  QVariantMap plugins = package.getPlugins( "world", "value" );
  std::map<QString, std::vector<QString>> expected_plugins = {
      { "rich_people", { "Nothing but despair" } }, { "rick_astley", { "Never Gonna Give You Up" } } };

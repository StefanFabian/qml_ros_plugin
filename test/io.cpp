...
TEST( IO, yaml )
{
  ...
  ASSERT_FALSE( io.writeYaml( "/usr/bin/i_really_hope_this_fails_on_your_machine_for_your_own_sake.mission", map ));
  ...
  file = io.readYaml( "/usr/bin/i_really_hope_this_fails_on_your_machine_for_your_own_sake.mission" );
  EXPECT_EQ( file.type(), QVariant::Bool );
  EXPECT_FALSE( file.toBool());
...
QtObject {
  ...
  std::vector<std::tuple<QVariant::Type, double, QVariant::Type, double, QVariant::Type, double>> expected_points = {

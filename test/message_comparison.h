...
template<typename T>
::testing::AssertionResult mapAndMessageEqual( const QVariant &map, T msg,
                                               const std::string &path = "msg", double = 1E-10 )
{
  if ( map.value<T>() == msg )
...
::testing::AssertionResult mapAndMessageEqualArray( const ContainerTypeA &a, const ContainerTypeB &b,
...
::testing::AssertionResult
mapAndMessageEqual( const QVariant &map, const boost::array<ElementType, N> &msg,
                    const std::string &path = "msg", double precision = 1E-10 )
{
  if ( map.type() == QVariant::List ) {
    return mapAndMessageEqualArray( map.toList(), msg, path, precision );
...
template<>
::testing::AssertionResult mapAndMessageEqual( const QVariant &map, std::string msg,
...
template<>
::testing::AssertionResult mapAndMessageEqual( const QVariant &map, ros::Time msg,
                                               const std::string &path, double precision )
{
  if ( std::abs( map.value<Time>().toSec() - msg.toSec() ) <= precision )
    return ::testing::AssertionSuccess();
  return ::testing::AssertionFailure() << "Map at " << path << " differed!" << std::endl
...
template<>
::testing::AssertionResult mapAndMessageEqual( const QVariant &map, ros::Duration msg,
                                               const std::string &path, double precision )
{
  bool ok;
  if ( std::abs( map.toDouble( &ok ) - msg.toSec() * 1000 ) <= precision && ok )
    return ::testing::AssertionSuccess();
  if ( !ok )
    return ::testing::AssertionFailure() << "Double cast at " << path << " not ok!";
  return ::testing::AssertionFailure() << "Map at " << path << " differed!" << std::endl
...
template<>
::testing::AssertionResult mapAndMessageEqual( const QVariant &map, std_msgs::Header msg,
                                               const std::string &path, double precision )
{
  ...
  if ( std::abs( map.toMap()["stamp"].value<Time>().toSec() - msg.stamp.toSec() ) > precision )
    return ::testing::AssertionFailure()
           ...
           << "Map: " << map.toMap()["stamp"].value<Time>().toSec() << std::endl
           << "Message: " << msg.stamp.toSec();
  return ::testing::AssertionSuccess();
}

template<>
::testing::AssertionResult mapAndMessageEqual( const QVariant &map, TestMessage msg,
                                               const std::string &path, double precision )
{
  if ( map.toHash()["header"].toHash()["frame_id"].toString().toStdString() != msg.header.frame_id )
    return ::testing::AssertionFailure()
           << "Map at " << path << ".header.frame_id  differed!" << std::endl
           << "Map: " << map.toHash()["header"].toHash()["frame_id"].toString().toStdString()
           << std::endl
           << "Message: " << msg.header.frame_id;
  if ( std::abs( map.toHash()["header"].toHash()["stamp"].value<Time>().toSec() -
                 msg.header.stamp.toSec() ) > precision )
    return ::testing::AssertionFailure()
           << "Map at " << path << ".header.stamp differed!" << std::endl
           << "Map: " << map.toHash()["header"].toHash()["stamp"].value<Time>().toSec() << std::endl
...
template<>
::testing::AssertionResult mapAndMessageEqual( const QVariant &map, TestArray msg,
                                               const std::string &path, double precision )
{
  ::testing::AssertionResult result = ::testing::AssertionSuccess();
  if ( !( result =
...
template<>
::testing::AssertionResult mapAndMessageEqual( const QVariant &map, geometry_msgs::Point msg,
                                               const std::string &path, double precision )
{
  bool ok;
                                         ...
                                         const std::string &path = "msg", double precision = 1E-10 )
{
  if ( compound["header"]["frame_id"].value<std::string>() != msg.header.frame_id )
    return ::testing::AssertionFailure()
           << "Retranslated message differed at " << path << ".header.frame_id !" << std::endl
           << "Map: " << compound["header"]["frame_id"].value<std::string>() << std::endl
           << "Message: " << msg.header.frame_id;
  if ( std::abs( compound["header"]["stamp"].value<ros::Time>().toSec() - msg.header.stamp.toSec() ) >
       precision )
    return ::testing::AssertionFailure()
           << "Retranslated message differed at " << path << ".header.frame_id!" << std::endl
  ...
  if ( std::abs( compound["d"].value<ros::Duration>().toSec() - msg.d.toSec() ) >= 1E-3 )
    return ::testing::AssertionFailure()
           << "Map at " << path << ".d differed!" << std::endl
...
template<>
::testing::AssertionResult mapAndMessageEqual( const QVariant &map,
                                               const roscpp_tutorials::TwoIntsRequest msg,
                                               const std::string &path, double )
{
  bool ok;
                                         ...
                                         const std::string &path, double precision = 1E-10 )
{
  ...
  ::testing::AssertionResult result = ::testing::AssertionSuccess();
  for ( size_t i = 0; i < arr.length(); ++i ) {
    if ( !( result = messageEqual( arr[i].as<CompoundMessage>(), msg[i],
                                   path + "[" + std::to_string( i ) + "]", precision ) ) )
      return result;
  }
  return result;
}
...
::testing::AssertionResult messageEqual( const CompoundArrayMessage &arr,
                                         const boost::array<TestSubArray, N> &msg,
                                         const std::string &path, double precision = 1E-10 )
{
  if ( arr.length() != msg.size() )
    return ::testing::AssertionFailure() << "Size of " << path << " differed!" << std::endl
                                         ...
                                         const std::string &path = "msg", double precision = 1E-10 )
{
  ...
  if ( !( result = messageEqual( compound["durations"].as<ArrayMessage<ros::Duration>>(),
    ...
    return result;
  if ( !( result = messageEqual( compound["times"].as<ArrayMessage<ros::Time>>(), msg.times,
  ...
  if ( !( result = messageEqual( compound["int32s"].as<ArrayMessage<int32_t>>(), msg.int32s,
    ...
    return result;
  if ( !( result = messageEqual( compound["int64s"].as<ArrayMessage<int64_t>>(), msg.int64s,
                                 path + ".int64s", precision ) ) )
    return result;
  ...
  if ( !( result = messageEqual( compound["strings"].as<ArrayMessage<std::string>>(), msg.strings,
    ...
    return result;
  if ( !( result = messageEqual( compound["subarrays_fixed"].as<CompoundArrayMessage>(),
                                         ...
                                         const std::string &path, double precision )
{
  ...
  if ( !( result = messageEqual( compound["strings"].as<ArrayMessage<std::string>>(), msg.strings,
                                 path + ".strings", precision ) ) )
    return result;
  return messageEqual( compound["times"].as<ArrayMessage<ros::Time>>(), msg.times, path + ".times",
                       precision );
}

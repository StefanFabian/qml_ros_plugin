...
GoalHandle::GoalHandle( std::shared_ptr<actionlib::ActionClient<ros_babel_fish::BabelFishAction>> client,
...
bool GoalHandle::expired() const
{
  return goal_handle_.isExpired();
}

void GoalHandle::resend()
{
  goal_handle_.resend();
}
...
{
  return static_cast<qml_ros_plugin::action_comm_states::CommState>( goal_handle_.getCommState().state_ );
}

TerminalState GoalHandle::terminalState() const
{
  const actionlib::TerminalState &state = goal_handle_.getTerminalState();
  return { static_cast<action_terminal_states::TerminalState>(state.state_), QString::fromStdString( state.text_ ) };
}

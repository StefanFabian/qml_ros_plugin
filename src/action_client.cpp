...
void ActionClient::onRosInitialized()
{
  ...
  {
    ROS_ERROR_NAMED( "qml_ros_plugin", "The action type did not end with 'Action'. Are you sure '%s' is an action?",
  ...
  if ( description == nullptr )
  {
    ROS_ERROR_NAMED( "qml_ros_plugin",
                     "Failed to look up action message of type '%s'. ActionClient not initialized and will not connect.",
...
void ActionClient::onRosShutdown()
{
  client_.reset();
}
...
void ActionClient::invokeTransitionCallback( QJSValue callback,
                                             actionlib::ActionClient<BabelFishAction>::GoalHandle handle )
{
